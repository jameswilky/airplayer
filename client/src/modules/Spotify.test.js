...
describe("Spotify Web API Module", () => {
  describe("search", () => {
    it("should  search for an artist", () => {
      ...
      expect(query.queryString).toBe("search?q=tania%20bowra&type=artist");
    });
    it("should search for an artist using a wildcard", () => {
      ...
      expect(query.queryString).toBe("search?q=tania*&type=artist");
    });
    it("should search for artists matching a given name, offset and limit", () => {
      ...
      expect(query.queryString).toBe(
        "search?q=bob&type=artist&limit=2&offset=20"
      );
    });
    ...
    it("should search for an album by a specific UPC code", () => {
      ...
      expect(query.queryString).toBe("search?q=upc:00602537817016&type=album");
    });
    it("should search for playlists matching a given name or description matching a string", () => {
      ...
      expect(query.queryString).toBe(`search?q=doom%20metal&type=playlist`);
    });
    ...
    it("should search for tracks available only in a specific market", () => {
      ...
      expect(query.queryString).toBe("search?q=abba&type=track&market=US");
    });
  });
  describe("find", () => {
    describe("albums", () => {
      it("should find an album matching the given id", () => {
        ...
        expect(query.queryString).toBe("albums/0sNOF9WDwhWunNAHPD3Ba");
      });
      it("should find the tracks of an album matching the given id", () => {
        const query = spotify.find({
          tracks: { where: { albums: { id: "6akEvsycLGftJxYudPjmqK" } } }
        });
        expect(query.queryString).toBe("albums/6akEvsycLGftJxYudPjmqK/tracks");
      });
    ...
    describe("artists", () => {
      it("should find a single artists matching the id", () => {
        ...
        expect(query.queryString).toBe("artists/0OdUWJ0sBjDrqHygGUXeCF");
      });
      ...
      it("should find the topTracks of an artist matching the given id", () => {
        ...
        expect(query.queryString).toBe(
          "artists/43ZHCT0cAZBISjO8DG9PnE/top-tracks"
        );
      });
    ...
    describe("tracks", () => {
      it("should find the audio analysis for a track matching the given id", () => {
        ...
        expect(query.queryString).toBe("audio-analysis/3JIxjvbbDrA9ztYlNcp3yL");
      });
      it("should find the audio features for a track matching the given ids", () => {
        ...
        expect(query.queryString).toBe("audio-features/3JIxjvbbDrA9ztYlNcp3yL");
      });
      ...
      it("should find a single track matching the given id", () => {
        ...
        expect(query.queryString).toBe("tracks/11dFghVXANMlKmJXsNCbNl");
      });
    });
  });
  describe("user", () => {
    describe("following", () => {
      ...
      it("should follow a user", () => {
        ...
        expect(query.queryString).toBe(
          "me/following?type=user&ids=exampleuser01"
        );
      });
      ...
      it("should follow a playlist", () => {
        ...
        expect(query.queryString).toBe(
          "playlists/2v3iNvBX8Ay1Gt2uXtUKUT/followers"
        );
      });
      ...
      it("should get the current users artists that they are following", () => {
        ...
        expect(query.queryString).toBe("me/following?type=artist");
      });
      ...
      it("should unfollow the target playlist", () => {
        ...
        expect(query.queryString).toBe(
          "playlists/65V6djkcVRyOStLd8nza8E/followers"
        );
      });
    });
    describe("library", () => {
      ...
      it("should get all tracks in the users library", () => {
        const query = spotify
          ...
          .library()
          .get("tracks");
        expect(query.queryString).toBe("/me/tracks");
      });
      it("should get all albums in the users library", () => {
        const query = spotify
          ...
          .library()
          .get("albums");
        expect(query.queryString).toBe("/me/albums");
      });
      ...
      it("should return top artists", () => {
        ...
        expect(query.queryString).toBe(`/me/top/tracks`);
      });
    });
  });
